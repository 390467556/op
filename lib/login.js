...
casper.start('http://open.oppomobile.com/newuser/login', function() {
    console.log('登录.....');
    this.captureSelector(dir + '/checkCode.jpg', '#yw0', {
        quality: 100
    });
});

casper.waitFor(function check() {
    return fs.exists(dir + '/checkCodeText.txt');
}, function then() {
    if (fs.exists(dir + '/checkCodeText.txt')) {
        var checkCode = fs.read(dir + '/checkCodeText.txt').trim();
